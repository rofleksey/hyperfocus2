...
Usage:
    python fastocr.py data/*.jpg                 # names, one per line
    python fastocr.py --json data/*.jpg          # {"image":..,"names":[..]}
    python fastocr.py --workers 8 --json *.jpg   # parallel (2500 in ~5 min)
    python fastocr.py --test                     # accuracy vs data/*.json
"""
...
def process(engine, img_path: str, hybrid: bool = False) -> list[str]:
    if hybrid:
        return _process_hybrid(engine, img_path)
    rec = preprocess(img_path)
...
def _process_hybrid(engine, img_path: str) -> list[str]:
