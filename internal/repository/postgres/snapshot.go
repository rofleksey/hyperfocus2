...
FROM (
  SELECT id, taken_at, stream_count, duration_seconds, disk_usage_bytes FROM snapshots
  ORDER BY taken_at DESC LIMIT $1
) s
LEFT JOIN stream_samples ss ON ss.snapshot_id = s.id
GROUP BY s.id, s.taken_at, s.stream_count, s.duration_seconds, s.disk_usage_bytes, s.disk_usage_bytes
...
func (r *Repository) InsertSnapshot(ctx context.Context, takenAt time.Time, source string, count int, durationSecs float64, diskBytes int64) (int64, error) {
	var id int64
	err := r.db(ctx).QueryRow(ctx, `
INSERT INTO snapshots (taken_at, source, stream_count, duration_seconds, disk_usage_bytes) VALUES ($1, $2, $3, $4, $5) RETURNING id;`,
		takenAt, source, count, durationSecs, diskBytes).Scan(&id)
	return id, err
}
...
SELECT id, taken_at, source, stream_count, duration_seconds, disk_usage_bytes FROM snapshots
WHERE taken_at <= $1 ORDER BY taken_at DESC LIMIT 1;`, t)
	var s entity.Snapshot
	if err := row.Scan(&s.ID, &s.TakenAt, &s.Source, &s.StreamCount, &s.DurationSeconds, &s.DiskUsageBytes); err != nil {
...
ORDER BY taken_at DESC LIMIT $3;`, from, to, limit)
	...
	for rows.Next() {
		var s entity.Snapshot
		if err := rows.Scan(&s.ID, &s.TakenAt, &s.Source, &s.StreamCount, &s.DurationSeconds, &s.DiskUsageBytes); err != nil {
			return nil, err
		}
		out = append(out, s)
	}
